    ...
    <div className="flex flex-col h-full w-full bg-white">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border-b border-slate-200 shrink-0">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="text-xs font-bold text-slate-800 whitespace-nowrap">OpenCut Editor</span>
          <form
          ...
          >
            <input
              ...
              className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-800 placeholder-slate-400 outline-none focus:border-cyan-500/40 font-mono"
            />
          </form>
        </div>
        ...
        {error || (loading && isLocalhost) ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-white z-10 p-8">
            ...
            <div className="text-center max-w-md">
              <h3 className="text-base font-bold text-slate-800 mb-2">OpenCut Editor</h3>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-left mb-4">
                <p className="text-xs font-semibold text-slate-700 mb-2">Bước 1 — Cài OpenCut:</p>
                <code className="block text-xs bg-white border border-slate-200 rounded-lg px-3 py-2 font-mono text-slate-800 mb-3">
                  git clone https://github.com/nichochar/opencut.git<br/>
                  cd opencut/apps/web && bun install
                </code>
                <p className="text-xs font-semibold text-slate-700 mb-2">Bước 2 — Chạy server:</p>
                <code className="block text-xs bg-white border border-slate-200 rounded-lg px-3 py-2 font-mono text-slate-800">
                  bun dev
                </code>
